class Fruit {
    ...
    render() {
        ...
        this.ctx.quadraticCurveTo(stalk_root.x, stalk_root.y - offset, stalk_root.x + 2 * offset, stalk_root.y - 3 * offset);
        this.ctx.quadraticCurveTo(stalk_root.x + 4 * offset, stalk_root.y - 6 * offset, stalk_root.x + 6 * offset, stalk_root.y - 6 * offset);
        this.ctx.stroke();

        // Leaf
        this.ctx.beginPath();
        this.ctx.moveTo(stalk_root.x + 2 * offset, stalk_root.y - 3 * offset);
        this.ctx.quadraticCurveTo(stalk_root.x, stalk_root.y - 7 * offset, stalk_root.x - 4 * offset, stalk_root.y - 5 * offset);
        this.ctx.quadraticCurveTo(stalk_root.x - 2 * offset,  stalk_root.y - 3 * offset, stalk_root.x + 2 * offset,  stalk_root.y - 3 * offset);
